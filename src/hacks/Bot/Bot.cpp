...
namespace eclipse::hacks::Bot {
    ...
    // set whenever playback is (re-)enabled; tells processBot() to sync
    // currently-held buttons for the frame playback started at, instead of
    // bursting through every skipped input one by one
    static bool s_needsSync = false;
    ...
    class $hack(Bot) {
        ...
        void init() override {
            const auto updateBotState = [](int state) {
                ...
                if (state == (int)bot::State::PLAYBACK) {
                    // playback may be starting mid-level (e.g. from a practice
                    // checkpoint), so sync held buttons on the next frame
                    // instead of assuming we're starting from frame 0
                    s_needsSync = true;
                }
            };
    ...
    class $modify(BotBGLHook, GJBaseGameLayer) {
        ...
        void processBot() {
            ...
            if (s_needsSync) {
                ...
                // instantly apply whatever buttons should already be held as of
                // this frame, instead of firing every skipped click/release
                // one after another (which used to cause a burst of stray
                // clicks/particles/triggers when starting playback mid-level).
                // note: this only folds in frames strictly BEFORE progress —
                // anything recorded exactly on the starting frame is left for
                // the poll() loop below, so it still fires individually and in
                // order instead of being collapsed into the held-state check.
                for (auto& held : s_bot.seekToFrame(progress - 1)) {
                    #ifdef GEODE_IS_MOBILE
                    m_allowedButtons.clear();
                    #endif
                    this->simulateClick((PlayerButton) held.button, held.down, held.player2);
                }
            }
